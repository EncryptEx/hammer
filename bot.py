...
async def SetWarning(userid: int,
                     substractMode: bool,
                     wantsToWipeAllWarns: bool = False):
    cur.execute("SELECT * FROM warns WHERE userid=? LIMIT 1", (userid, ))
    rows = cur.fetchall()
    # print(rows)
    if len(rows) > 0:
...
async def clearwarns(ctx, member: discord.Member, *, reason=None):
    ...
    embed = Embed(
        title=
        f":partying_face: The warns of {member} have been removed! :hammer_pick:",
        description=descr,
    )
    embed.set_footer(
        text=f"Hammer | Command executed by {ctx.author}",
    ...
    embed.set_thumbnail(url=member.display_avatar)
    warn = await SetWarning(member.id,
                            substractMode=True,
                            wantsToWipeAllWarns=True)
    embed.add_field(
        name="Warn count",
